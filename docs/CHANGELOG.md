...
- **Auth + Intake Readability Pass**: Registration and verification onboarding cards, verification code tracking, report slider helper labels, digest score labels, and encryption/support copy were softened onto calmer surfaces and more resilient text scales for better mobile readability. (cross-app UI polish)
- **Shared Label + Auth Surface Standardization**: Shared field/stat labels now use calmer tracking, login and unauthorized hero surfaces now inherit the same semantic panel treatment as the rest of the app, and My Contributions now follows the same page-header and protocol-meta pattern as other primary views. (cross-app UI polish)
- **High-Density Label Calm Pass**: Dashboard, Community Home, Signal Detail, and Settings now use the same calmer uppercase label tracking instead of the most aggressive spaced-out treatment, improving readability and reducing wrapping stress in dense community views. (cross-app UI polish)
