            ...
            <div className="animate-fade-in">
              ...
              <Card className="shadow-8 border-1 border-white-alpha-10 bg-gray-900 overflow-hidden">
                <DataTable value={flagged} loading={loading} emptyMessage={t('moderation.empty')} className="p-datatable-sm">
                  <Column field="title" header={t('moderation.suspect_title')} body={(s: Signal) => <span className="font-bold text-white">{s.title}</span>} />
                  <Column field="category" header={t('common.category')} body={(s: Signal) => <Tag value={s.category} severity="secondary" />} />
                              <Column field="priorityScore" header={t('common.score')} body={(s: Signal) => <span className="font-mono text-cyan-400 font-bold">{s.priorityScore?.toFixed(0)}</span>} />
                              <Column header={t('common.actions')} body={(s: Signal) => (
