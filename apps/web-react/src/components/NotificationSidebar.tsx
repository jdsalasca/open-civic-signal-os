          ...
          notifications.map((n) => (
            <div key={n.id} className="p-3 bg-white-alpha-5 border-round-lg border-1 border-white-alpha-10">
              <div className="flex justify-content-between align-items-center mb-2">
                ...
                <span className="text-xs text-gray-600 font-mono">{new Date(n.sentAt).toLocaleDateString()}</span>
              </div>
              <p className="text-sm text-gray-300 m-0 line-height-3">{n.message}</p>
            </div>
          ))
        )}
      </div>
